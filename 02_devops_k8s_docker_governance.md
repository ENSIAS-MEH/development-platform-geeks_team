...
| React Frontend       | Member 4 | Member 4 | Member 4 | Member 4 |
...
| React/Vite Frontend | 80 | 3000 | Member 4 |
...
| React/Vite (build stage) | `node:20-alpine` | Build frontend assets |
| React/Vite (runtime) | `nginx:1.27-alpine` | Efficient static asset serving |

> **RULES:**
> - Never use `latest` tag on base images.
> - Always pin versions explicitly.
> - Always use Alpine variants.
...
### 3.3 Standard Dockerfile — React + Vite Frontend
...
```
### 3.3.1 Standard `nginx.conf` — React/Vite Frontend
...
> IMPORTANT:
> - Vite proxy configuration only works during local development.
> - In Docker/Kubernetes production deployments, Nginx handles API forwarding.
...
Vite proxy configuration forwards API calls to backend services during development.
...
---
### 11.3 Frontend Local Development
...
The frontend is deployed as static assets served by Nginx.
Node.js is used only during build time and is never shipped to production.
